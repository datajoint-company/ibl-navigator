...
export class PlotsService {
    ...
    getPlots(type, id) {
        this.http.get(`http://localhost:3333/plots/${ type }/${ id }`)
        .subscribe((plotData) => {
            this.plots = plotData;
    ...
    retrievePlot(plotInfo) {
        ...
        this.http.post(`http://localhost:3333/plot/`, plotInfo, { responseType: 'text' })
            .subscribe(
                (plotData) => {
                    this.plots = plotData;
    ...
    testPost(someData) {
        ...
        this.http.post('http://localhost:3333/mice/', someData, httpOptions)
            .subscribe(
                (x) => {
                    this.somePost = x;
                    console.log('plots (in service retrievePlot) are: ');
    ...
    getSamplePlot() {
        this.http.get(`http://localhost:3333/plots/testPlot`)
        .subscribe((plotData) => {
